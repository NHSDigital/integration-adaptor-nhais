...
public class SchedulerTimestampRepositoryExtensionsImpl implements SchedulerTimestampRepositoryExtensions {
    ...
    public boolean updateTimestamp(String schedulerType, Instant timestamp, long seconds) {
        ...
        if (documentAlreadyExists(schedulerType)) {
            ...
            LOGGER.info(
                "{} collection does not exist or it is empty. Document with timestamp will be created",
            ...
            try {
                ...
                LOGGER.warn(
                    "Unable to create new document for scheduler type "
                        + schedulerType
                        + ". Most likely another instance already created the document.",
                    e
                );
            }
            return false;
    ...
    private boolean documentAlreadyExists(String schedulerType) {
        ...
        if (count > 1) {
            LOGGER.error(
                "More than one document exists for schedulerType {}. This can cause unexpected scheduling behaviour.",
                schedulerType
            );
        }
        return count >= 1;
